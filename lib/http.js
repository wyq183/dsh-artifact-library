    ...
    try {
      ...
      if (!isLoopback(req)) {
        const localOnly =
          req.method !== 'GET' ||
          id === 'export' ||
          (id !== '' && !['search', 'stats', 'categories', 'settings', 'model-catalog', 'suggest-cleanup'].includes(id) && !id.endsWith('/related'))
        if (localOnly) return sendJson(res, 403, { error: '该操作仅限本机（127.0.0.1）使用，局域网只读浏览请使用列表视图' })
      }
      ...
      if (req.method === 'GET' && id === 'model-catalog') {
        if (!deps.modelCatalog) return sendJson(res, 500, { error: '模型目录不可用' })
        try {
          return sendJson(res, 200, await deps.modelCatalog())
        } catch (e) {
          return sendJson(res, 500, { error: `模型目录读取失败: ${e?.message || e}` })
        }
      }
