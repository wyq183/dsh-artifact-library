...
export function artifactsHandler(store, deps = {}) {
  return async (req, res) => {
    try {
      ...
      // P3 助手会话：POST /ext/artifacts/assistant-session
      if (req.method === 'POST' && id === 'assistant-session') {
        const { openAssistantSession } = await import('./assistant-session.js')
        if (!deps.openAssistantSession) return sendJson(res, 500, { error: '助手会话功能未启用' })
        const result = await deps.openAssistantSession()
