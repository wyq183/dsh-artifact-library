...
export class ArtifactStore {
  ...
  async importFolder(dirPath, { project = '', maxFiles = 10000, maxSizeBytes = 50 * 1024 * 1024 } = {}) {
    ...
    const PROTECTED_DIRS = new Set(['.ssh', '.gnupg', '.aws', '.azure', '.kube', '.docker', '.npmrc', 'appdata', 'windows', 'system32'])
    if (PROTECTED_DIRS.has(baseName) || /^\.dsh$/i.test(baseName)) {
    ...
    // 敏感文件名：凭据/密钥/证书/env 等绝不进库（防止 API key 泄露进全文索引）
    const SENSITIVE_FILE_RE = /(^|[._-])(credentials?|secret|token|apikey|api[_-]?key|passwd|password|auth|\.env|\.pem|\.key|\.p12|\.pfx|id_rsa|id_ed25519|\.npmrc|\.netrc|\.git-credentials)([._-]|$)/i
    const isSensitiveFile = (name) => SENSITIVE_FILE_RE.test(name)
      ...
      for (const e of entries) {
        ...
        try {
          if (e.isDirectory()) {
            ...
            if (isSensitiveFile(e.name)) { skipped++; continue } // 凭据/密钥绝不进库
