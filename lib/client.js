...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var ASST_CLASS = "dsh-artifact-library-assistant";

    var css = [
      "." + BUTTON_CLASS + ",." + ASST_CLASS + "{",
      ...
      "." + BUTTON_CLASS + ":hover,." + ASST_CLASS + ":hover{",
      "  background:var(--dsw-alias-interactive-bg-hover-solid, rgba(127,160,255,.16));",
      "  border-color:var(--dsw-alias-brand-primary, #4da3ff);",
      "}",
      "." + BUTTON_CLASS + " .alf-icon,." + ASST_CLASS + " .alf-icon{font-size:16px;line-height:1}",
      "." + BUTTON_CLASS + " .alf-label,." + ASST_CLASS + " .alf-label{flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
      "body[data-ds-collapsed] ." + BUTTON_CLASS + ",body[data-ds-collapsed] ." + ASST_CLASS + "{display:none}",
    ].join("\n");
    ...
    // 「问产物库」：调助手会话端点（P3），然后跳管理页让用户点开会话
    function makeAssistantButton() {
      var btn = document.createElement("div");
      btn.className = ASST_CLASS;
      btn.setAttribute("role", "button");
      btn.setAttribute("tabindex", "0");
      btn.title = "用自然语言问产物库：创建/复用「产物库助手」会话";
      btn.innerHTML = '<span class="alf-icon">🤖</span><span class="alf-label">问产物库</span>';
      var busy = false;
      var act = function () {
        if (busy) return;
        busy = true;
        btn.style.opacity = ".55";
        fetch("/ext/artifacts/assistant-session", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" })
          .then(function (r) { return r.json(); })
          .then(function (j) {
            if (j && j.ok) {
              window.open("/ext/artifact-library/?assistant=" + encodeURIComponent(j.sessionId), "_blank");
            } else {
              alert("❌ 助手会话启动失败：" + ((j && j.error) || "未知错误"));
            }
          })
          .catch(function (e) { alert("❌ 网络错误：" + e.message); })
          .finally(function () { busy = false; btn.style.opacity = "1"; });
      };
      btn.addEventListener("click", act);
      btn.addEventListener("keydown", function (e) { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); act(); } });
      return btn;
    }
    ...
    function apply(ctx) {
      ...
      var timer = setInterval(function () {
        ...
        if (!col) return;
        if (!col.querySelector("." + BUTTON_CLASS)) {
          var btn = makeButton();
          // 插到设置脚正上方（工作区列表与设置之间）
        ...
        if (!col.querySelector("." + ASST_CLASS)) {
          var ab = makeAssistantButton();
          var ref = col.querySelector("." + BUTTON_CLASS) || col.querySelector(".mSGvra_footArea") || col.querySelector(".mSGvra_settingsArea");
          if (ref && ref.parentElement) {
            ref.parentElement.insertBefore(ab, ref.nextSibling || null);
          } else {
            col.appendChild(ab);
          }
        }
      ...
      return function () {
        ...
        if (typeof document !== "undefined") {
          var nodes = document.querySelectorAll("." + BUTTON_CLASS + ",." + ASST_CLASS);
