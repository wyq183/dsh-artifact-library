...
**修复 · 目录型记录被误报为「文件缺失」，且点整理消不掉：**

- 根因：登记时 `probePath` 用 `statSync` 成功即算存在（目录也算），而整理时 `runCleanup` 用 `st.isFile()` 判定——目录一律为 false，于是被标成「文件缺失」。两处判定不一致，导致点多少次「🧹 立即整理一次」都消不掉
- 现在 `probePath` 与 `runCleanup` 统一为「路径存在即存在」，并新增 `is_dir` 字段；`stats.missingFiles` 与整理建议单的「文件缺失」不再包含目录
- 老记录在 `load()` 回填 `is_dir`，整理时按实际类型修正

**改进 · 缺失记录可处理（原提示有误导）：**

- 顶部提示文案改为「整理只能刷新状态、找不回文件；可在整理建议里归档或删除记录」（原文案暗示整理能解决）
- 「🧹 整理建议」的文件缺失组新增：每条「归档」按钮 + 「归档全部缺失记录」批量按钮（归档只动记录，不动文件）

**测试：** test-fixes 增目录断言（登记目录 `is_dir=true` / 整理后不再误报 / 缺失统计不含目录）；四个测试文件全绿。
